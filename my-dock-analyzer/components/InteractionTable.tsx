        ...
        <table className="w-full text-sm text-left text-slate-600">
          <thead className="text-xs text-slate-700 uppercase bg-slate-50 sticky top-0 z-10 shadow-sm">
            <tr>
              ...
              <th className="px-4 py-2 border-b">Dist (Å)</th>
              <th className="px-4 py-2 border-b">Ligand Atom</th>
              <th className="px-4 py-2 border-b">Protein Res</th>
              <th className="px-4 py-2 border-b">Protein Atom</th>
            </tr>
          </thead>
            ...
            {sortedInteractions.map((interaction) => {
               ...
               return (
                <tr key={interaction.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-4 py-2 font-mono text-slate-900 font-medium">{interaction.distance.toFixed(2)}</td>
                  <td className="px-4 py-2 text-slate-600">{interaction.ligandAtom.name} <span className="text-slate-400 text-xs">({interaction.ligandAtom.element})</span></td>
                  <td className="px-4 py-2 font-medium text-slate-800">
                    {interaction.proteinAtom.resName} {interaction.proteinAtom.resNo} <span className="text-slate-400 font-normal">({interaction.proteinAtom.chain})</span>
                  </td>
                  <td className="px-4 py-2 text-slate-600">{interaction.proteinAtom.name}</td>
                </tr>
              );
            })}
          </tbody>
